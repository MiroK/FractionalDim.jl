@test begin
    c = [Segment(Point(0, 0, 0), Point(1, 1, 1))]
...
@test begin
    a = collect(linspace(0, 2, 10000))
    c = [Segment(Point(a[i], a[i], a[i]), Point(a[i+1], a[i+1], a[i+1]))
...
@test begin
    θ = linspace(0, 2*pi, 10000)
    x = sin.(θ)
    y = cos.(θ)
    z = 0.5 + 0.5*x
    # Lift
    c = [Segment(Point(x[i], y[i], z[i]), Point(x[i+1], y[i+1], z[i+1])) for i in 1:10000-1]
